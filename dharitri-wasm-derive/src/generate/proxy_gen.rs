...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    if token_count > 0 || nonce_count > 0 || payment_count > 0 {
        ...
        if token_count == 0 && nonce_count == 0 {
            contract_call_type = quote! { dharitri_wasm::types::ContractCallWithMoax };
            contract_call_init = quote! {
                let mut ___contract_call___ = dharitri_wasm::types::ContractCallWithMoax::new(
                    ___address___,
                    #endpoint_name,
                    #payment_expr,
                );
            ...
            contract_call_type = quote! { dharitri_wasm::types::ContractCallWithMoaxOrSingleDct };
            contract_call_init = quote! {
                let mut ___contract_call___ = dharitri_wasm::types::ContractCallWithMoaxOrSingleDct::new(
                    ___address___,
                    #endpoint_name,
                    #token_expr,
                    #nonce_expr,
